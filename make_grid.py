...
def get_Alm(theta0, delta0, ftype, l):
	try:
		el, em, Alm=Alm_cpp(l, theta0=theta0, delta=delta0, ftype=ftype) # Array of m E [-l,l]
	except:
		print('ERROR ON ALM_CPP...')
		r=Alm_cpp(l, theta0=theta0, delta=delta0, ftype=ftype)
		print(r)
		exit()
	return Alm 

def make_Alm_grid(l=1, theta_min=0, theta_max=np.pi/2, delta_min=0, delta_max=np.pi/4, resol=np.pi/20, ftype='gauss', flatarray=False):
	...
	for theta0 in theta:
		...
		for delta0 in delta:
			#print('    (i, j)', '(',i, ',', j,')')
			#print('   tot: ', tot)
			v=get_Alm(theta0, delta0, ftype, l)
			#print('        ', v)
			if flatarray == False:
				Alm[:,i,j]=v
			else:
