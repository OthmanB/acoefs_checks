...
def eval_acoefs(l, nu_nls): # We expect nu_nls=[nu(-l), nu(-l+1), ... , nu[0], nu[1], ... nu(l)]
	...
	if l == 2: 
		...
		# a3 term:
		#a3=2*Tn22/Pslm(3,2,2) - a1*K_a1 # EXACT VERIFIED
		a3=(Tn22 - Tn21)/5  # TO VERIFY
		# a4 term:
		#Den_a2=-7 #Pslm(2,2,1) - Pslm(2,2,0) - (Pslm(4,2,1)-Pslm(4,2,0)) * (Pslm(2,2,2)-Pslm(2,2,0))/(Pslm(4,2,2)-Pslm(4,2,0))
		#a2=(Sn21 - (Pslm(4,2,1)-Pslm(4,2,0))/(Pslm(4,2,2)-Pslm(4,2,0)) *Sn22)/Den_a2
		a2=(2*Sn22 - Sn21)/7
		# a4 term:
		#a4=(Sn22 - a2*(Pslm(2,2,2)-Pslm(2,2,0)))/(Pslm(4,2,2) - Pslm(4,2,0))
		#a4=(-Sn22 + 4*a2)/10 # EXACT VERIFIED
		a4=(Sn22 - 4*Sn21)/70. 
		a5=0
		a6=0
	if l==3: # VERIFIED OK
		...
		# a3 term: 
		#a3=Tn32*B1/A1+ Tn33*C1/A1 + a1*D1/A1 # VERIFIED
		#a3=-14*a1/9 + 2*Tn32/9 + 4*Tn33/3
		a3=-Tn31/9 - 2*Tn32/9 + Tn33/3
		# a5 term: 
		#a5=(3*Tn33 - a1*Pslm(1,3,3) - a3*Pslm(3,3,3))/Pslm(5,3,3)
		#a5=(3*Tn33 - 3*Tn31/14 - 6*Tn32/7 - 27*Tn33/14 + Tn31/3 + 2*Tn32/3 - Tn33)/3
