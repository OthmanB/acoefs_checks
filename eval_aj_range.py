...
def eval_aj_mean_range(Mass=1., Radius=1., Teff=5777, epsilon_nl=5*1e-4, a1=1000, dir_core='/Users/obenomar/tmp/test_a2AR/'):
	'''
	...
	for j in range(len(theta)):
		for k in range(len(delta)):
			...
			for l in range(1,lmax+1):
				...
				#nu_nlm_all.append([l,nu_nlm]) # Keep memory of all groups of modes (for debug)
	...
	#for nu in nu_nlm_all:
	#	print("l=", nu[0], '   nu_nlm=', nu[1])

	return [a2_min*1e3, a2_max*1e3], [a4_min*1e3, a4_max*1e3] # aj converted into nHz
...
for j in range(len(M)):
	...
	for dnu in a1:
		...
		a2_range, a4_range=eval_aj_mean_range(Mass=M[j], Radius=R[j], Teff=T[j], epsilon_nl=epsilon_nl, a1=dnu)
